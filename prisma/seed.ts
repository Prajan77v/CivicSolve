...
async function main() {
  ...
  // 5 NEW COMPLETELY UNIQUE DOMAIN-DISTINCT CHALLENGES
  const p16 = await prisma.problem.create({ data: { title: 'Toxic Heavy Metal Leakage & Hazardous E-Waste Dumping in Peenya Industrial Zone', description: 'Over 120 e-waste dismantlers in Peenya, Bengaluru dispose of circuit boards by acid leaching into stormwater drains. Lead, cadmium, and mercury levels in soil are 18x safe environmental limits. An automated robotic sorting and hydrometallurgical recovery system is required.', category: 'ELECTRONICS_WASTE', subcategory: 'Heavy Metal Recovery', priority: 'CRITICAL', status: 'DEPLOYED', submittedById: citizen1.id, affectedCount: 140000, tags: JSON.stringify(['e-waste', 'heavy metals', 'Bengaluru', 'recycling', 'hydrometallurgy']), sdgGoals: JSON.stringify(['12', '3', '6', '11']) } })
  await prisma.problemLocation.create({ data: { problemId: p16.id, address: 'Peenya 3rd Stage Industrial Area, Bengaluru', district: 'Bengaluru Urban', state: 'Karnataka', pincode: '560058', lat: 13.0315, lng: 77.5186 } })
  await prisma.problemAIAnalysis.create({ data: { problemId: p16.id, domain: 'Hazardous Waste', subdomain: 'E-Waste Recycling', confidence: 0.96, priorityScore: 0.98, priorityReason: 'CRITICAL: Lead and mercury soil toxicity 18x WHO threshold. Groundwater contamination threatening 140,000 residents in Peenya industrial hub.', affectedEstimate: 140000, duplicateRisk: 0.04, detectedTech: JSON.stringify(['Computer Vision Sorting', 'Hydrometallurgical Extraction', 'Heavy Metal IoT Sensors']), recommendedSkills: JSON.stringify(['Chemical Engineering', 'Robotics', 'Environmental Toxicology', 'IoT']), sdgAlignment: JSON.stringify(['12', '3', '6']), matchedUniversities: JSON.stringify([{name:'BITS Pilani',score:0.94},{name:'IIT Bombay',score:0.89}]), matchedTeams: JSON.stringify([{name:'GreenPath Solutions',score:0.95}]), matchedIndustry: JSON.stringify([{name:'Wipro EcoEnergy',score:0.92,type:'INDUSTRY'}]), nextSteps: JSON.stringify(['Install continuous heavy metal drain sensors', 'Deploy autonomous e-waste sorter', 'Bio-remediation of soil']) } })

  const p17 = await prisma.problem.create({ data: { title: 'Cyclone-Resilient Solar Microgrid & Saltwater Battery Bank for Sundarbans Islands', description: 'Ghoramara & Satjelia islands in Sundarbans lose grid power for 4-6 weeks during annual cyclones. Traditional lead-acid batteries corrode rapidly due to salt air. A modular saltwater battery solar microgrid is required to safeguard 28,000 islanders.', category: 'RENEWABLE_ENERGY', subcategory: 'Island Microgrids', priority: 'HIGH', status: 'PILOT', submittedById: citizen3.id, affectedCount: 28000, tags: JSON.stringify(['solar microgrid', 'Sundarbans', 'saltwater battery', 'cyclone resilience']), sdgGoals: JSON.stringify(['7', '13', '9']) } })
  await prisma.problemLocation.create({ data: { problemId: p17.id, address: 'Satjelia Island, Sundarbans Delta', district: 'South 24 Parganas', state: 'West Bengal', lat: 22.0253, lng: 88.8912 } })
  await prisma.problemAIAnalysis.create({ data: { problemId: p17.id, domain: 'Clean Energy', subdomain: 'Resilient Microgrids', confidence: 0.93, priorityScore: 0.91, priorityReason: 'HIGH: Island grid isolation leaves 28,000 inhabitants without power or medical cold chains during severe weather events.', affectedEstimate: 28000, duplicateRisk: 0.07, detectedTech: JSON.stringify(['Saltwater Battery Storage', 'Microgrid Inverters', 'Telemetry Gateway']), recommendedSkills: JSON.stringify(['Electrical Engineering', 'Solar Energy', 'Material Science', 'IoT']), sdgAlignment: JSON.stringify(['7', '13']), matchedUniversities: JSON.stringify([{name:'IIT Bombay',score:0.93},{name:'IIT Delhi',score:0.86}]), matchedTeams: JSON.stringify([{name:'SmartGrid Pioneers',score:0.94}]), matchedIndustry: JSON.stringify([{name:'Wipro EcoEnergy',score:0.88,type:'INDUSTRY'}]), nextSteps: JSON.stringify(['Pilot saltwater battery cell', 'Install storm-proof solar mountings', 'Integrate emergency health center backup']) } })

  const p18 = await prisma.problem.create({ data: { title: 'Autonomous Marine Skimmer & Bio-Enzymatic Microplastic Filter for Mandovi Estuary', description: 'Tourist activity and marine traffic discharge 2.4 tonnes of microplastics daily into Mandovi River estuary, threatening coastal fish hatcheries and tourism. Solar-powered autonomous marine skimmers with bio-enzymatic nylon degrading filters can restore water safety.', category: 'OCEAN_CONSERVATION', subcategory: 'Microplastics Remediation', priority: 'CRITICAL', status: 'ACTIVE', submittedById: citizen1.id, affectedCount: 95000, tags: JSON.stringify(['ocean', 'microplastics', 'Goa', 'Mandovi', 'enzymatic filter']), sdgGoals: JSON.stringify(['14', '6', '12']) } })
  await prisma.problemLocation.create({ data: { problemId: p18.id, address: 'Mandovi River Estuary, Panaji', district: 'North Goa', state: 'Goa', lat: 15.5007, lng: 73.8370 } })

  const p19 = await prisma.problem.create({ data: { title: 'IoT Subsurface Inclinometer & Acoustic Emission Landslide Predictor in Shimla', description: 'Unstable mountain slopes along NH-5 in Shimla suffer sudden landslides during monsoon monsoon rains, causing fatalities and road blockades. Subsurface wireless inclinometer sensors combined with acoustic emission monitoring provide 45-minute advance landslide warnings.', category: 'DISASTER_MANAGEMENT', subcategory: 'Landslide Warning', priority: 'CRITICAL', status: 'PROTOTYPE', submittedById: citizen2.id, affectedCount: 65000, tags: JSON.stringify(['landslide', 'Shimla', 'inclinometer', 'early warning', 'geotechnical']), sdgGoals: JSON.stringify(['11', '13', '9']) } })
  await prisma.problemLocation.create({ data: { problemId: p19.id, address: 'NH-5 Slope Bypass, Shimla', district: 'Shimla', state: 'Himachal Pradesh', lat: 31.1048, lng: 77.1734 } })

  const p20 = await prisma.problem.create({ data: { title: 'Modular Anaerobic Bio-Digester & Biogas Micro-Station in Dharavi Slum', description: 'High population density in Dharavi (800,000 residents) creates severe community toilet overflows and organic waste accumulation. Modular underground anaerobic bio-digesters convert organic waste into clean cooking gas and power local streetlights.', category: 'SANITATION', subcategory: 'Bio-Digester Waste Management', priority: 'CRITICAL', status: 'DEPLOYED', submittedById: citizen1.id, affectedCount: 800000, tags: JSON.stringify(['sanitation', 'biogas', 'Dharavi', 'Mumbai', 'bio-digester']), sdgGoals: JSON.stringify(['6', '11', '7', '3']) } })
  await prisma.problemLocation.create({ data: { problemId: p20.id, address: 'Sector 3, Dharavi, Mumbai', district: 'Mumbai Suburban', state: 'Maharashtra', lat: 19.0402, lng: 72.8509 } })
