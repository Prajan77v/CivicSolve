...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-20 flex h-14 w-full items-center justify-between border-b border-slate-200/80 dark:border-slate-800/80 bg-white/95 dark:bg-[#0b0f17]/90 px-3 sm:px-4 backdrop-blur-md md:px-6">
      {/* Left section: Mobile menu & Search bar */}
      <div className="flex items-center gap-2 md:gap-4 flex-1 min-w-0 max-w-xl">
        {/* Mobile Sidebar Hamburger */}
        <button
          onClick={toggleMobileSidebar}
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-100/80 dark:bg-[#111726] text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white lg:hidden active:scale-95 transition-all"
        ...
        <button
          ...
          className="group relative flex w-full min-w-0 items-center justify-between rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-[#111726] px-2.5 py-1.5 text-xs text-slate-500 dark:text-slate-400 transition-colors hover:border-slate-300 dark:hover:border-slate-700 hover:text-slate-900 dark:hover:text-slate-200 sm:w-80 md:w-96"
        >
          <div className="flex items-center gap-2 min-w-0 truncate">
            <Search className="h-3.5 w-3.5 text-slate-400 shrink-0" />
            <span className="truncate">Search challenges, teams, solutions...</span>
          </div>
          <kbd className="hidden sm:flex items-center gap-0.5 rounded border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800/60 px-1.5 py-0.5 text-[10px] font-medium text-slate-500 dark:text-slate-400 shrink-0">
            <span>⌘</span>K
          </kbd>
        </button>
      </div>
      ...
      <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
        ...
        <div className="relative" ref={appearanceRef}>
          <button
            ...
            className="relative flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-[#111726] text-slate-600 dark:text-slate-400 transition-colors hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white"
            aria-label="Customize appearance"
            title="Appearance & Theme"
          >
            {resolvedTheme === 'dark' ? (
              <Moon className="h-4 w-4" style={{ color: computedAccent.accent }} />
            ) : (
          ...
          {isAppearanceOpen && (
            <div
              ...
              className="absolute right-0 mt-2 w-72 max-w-[calc(100vw-1.5rem)] rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#111726] p-3 shadow-xl shadow-slate-900/10 dark:shadow-black/60 z-50 animate-fade-in space-y-3"
            >
        ...
        <div className="relative" ref={notificationsRef}>
          ...
          >
            ...
            {unreadNotificationsCount > 0 && (
              <span
                className="absolute -top-0.5 -right-0.5 flex h-4 min-w-4 items-center justify-center rounded-full px-1 text-[9px] font-bold text-white shadow-sm"
          ...
          {isNotificationsOpen && (
            <div className="absolute right-0 mt-2 z-50 animate-fade-in max-w-[calc(100vw-1.5rem)]">
        ...
        <div className="relative" ref={userMenuRef}>
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg p-1 transition-colors hover:bg-slate-100 dark:hover:bg-slate-800/80 active:scale-95"
            aria-label="User menu"
          >
            <div
              className="flex h-8 w-8 items-center justify-center rounded-lg text-xs font-semibold text-white shadow-sm overflow-hidden"
              style={{ backgroundColor: computedAccent.accent, color: computedAccent.foreground }}
            >
              {userAvatar ? (
          ...
          {/* Dropdown Menu */}
          {isUserMenuOpen && (
            <div className="absolute right-0 mt-2 w-64 max-w-[calc(100vw-1.5rem)] rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#111726] p-1.5 shadow-xl shadow-slate-900/10 dark:shadow-black/60 z-50 animate-fade-in">
              {/* User Header */}
