    ...
    if (token.startsWith('[') && token.includes('](')) {
      ...
      parts.push(
        <a
          key={match.index}
          href={href}
          className="text-cyan-400 underline underline-offset-2 hover:text-cyan-300 font-medium"
        >
          {label}
        </a>
      )
      ...
      parts.push(
        <code
          key={match.index}
          className="rounded bg-slate-800 px-1 py-0.5 font-mono text-[11px] text-cyan-300 border border-white/10"
        >
          {token.slice(1, -1)}
        </code>
      )
  ...
  return (
    <div className="space-y-2 leading-relaxed">
      {paragraphs.map((p, pIdx) => {
        if (p.startsWith('### ')) {
          return (
            <h4 key={pIdx} className="text-sm font-bold text-white pt-1 pb-0.5 border-b border-white/5">
        ...
        if (p.startsWith('#### ')) {
          return (
            <h5 key={pIdx} className="text-xs font-bold text-cyan-300 pt-1">
              {renderInline(p.replace('#### ', ''))}
            </h5>
          )
        }
        if (p.startsWith('> ')) {
          return (
            <blockquote
              key={pIdx}
              className="border-l-2 border-cyan-500/60 bg-cyan-950/20 px-3 py-1.5 text-slate-300 italic rounded-r"
            >
            ...
            <ul key={pIdx} className="space-y-1.5 my-1 pl-1">
              {lines.map((l, lIdx) => {
                ...
                if (numMatch) {
                  return (
                    <li key={lIdx} className="flex items-start gap-1.5 text-slate-200">
                      <span className="text-cyan-400 font-mono text-[10px] pt-0.5">{numMatch[1]}.</span>
    ...
    <AnimatePresence>
      {isAiOpen && (
        <div className="fixed inset-0 z-50 overflow-hidden font-sans">
          ...
          {/* Assistant Panel (Full screen on mobile, 520px on desktop) */}
          <motion.aside
            initial={{ x: '100%', opacity: 0 }}
            animate={{ x: 0, opacity: 1 }}
            exit={{ x: '100%', opacity: 0 }}
            transition={{ type: 'spring', damping: 28, stiffness: 260 }}
            className="fixed inset-y-0 right-0 z-50 flex w-full sm:max-w-xl flex-col border-l border-white/10 bg-[#080d1a] shadow-2xl shadow-black/90 text-slate-100"
          >
            {/* Header */}
            <div className="flex items-center justify-between border-b border-white/10 px-4 sm:px-5 py-3.5 bg-slate-900/80 backdrop-blur-md">
              <div className="flex items-center gap-3">
                <div className="relative flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-tr from-blue-600 via-indigo-600 to-cyan-400 p-0.5 shadow-[0_0_15px_rgba(6,182,212,0.35)]">
                  <div className="flex h-full w-full items-center justify-center rounded-[10px] bg-[#080d1a]">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-bold text-white tracking-tight">Civic AI</h3>
                    <span
                      className={cn(
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    showHistory ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
                  )}
                  title="Chat History"
            ...
            <AnimatePresence>
              {showHistory && (
                <motion.div
                  ...
                  className="border-b border-white/10 bg-slate-950/95 overflow-hidden"
                >
                  <div className="p-3">
                    <div className="flex items-center justify-between pb-2 border-b border-white/5">
                      <span className="text-xs font-semibold text-slate-300">Previous Conversations</span>
                      <button
                        type="button"
                        onClick={handleNewChat}
                    ...
                    <div className="mt-2 max-h-48 overflow-y-auto space-y-1 scrollbar-thin">
                      {conversations.length === 0 ? (
                        <p className="text-xs text-slate-500 py-3 text-center">No previous conversations saved.</p>
                      ) : (
                        conversations.map((c) => (
                          <div
                            key={c.id}
            ...
            {/* Chat Messages Body */}
            <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin bg-slate-950/30">
              {messages.map((msg, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={cn(
                        'max-w-[88%] rounded-2xl p-4 text-xs leading-relaxed group relative',
                        isAssistant
                          ? 'bg-slate-900/95 border border-white/10 text-slate-200 shadow-md'
                    ...
                    >
                      ...
                      {msg.metadata?.entities && msg.metadata.entities.length > 0 && (
                        <div className="mt-3 pt-2.5 border-t border-white/10 space-y-1.5">
                          <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 flex items-center gap-1">
                            <Layers className="h-3 w-3 text-cyan-400" /> Linked CivicSolve Records
                          </span>
                          <div className="grid grid-cols-1 gap-1.5">
                            {msg.metadata.entities.map((entity, eIdx) => (
                              <a
                                ...
                                href={entity.href}
                                onClick={() => setAiOpen(false)}
                                className="flex items-center justify-between p-2 rounded-lg bg-slate-950/70 border border-white/10 hover:border-cyan-500/50 hover:bg-slate-800 transition-all group/entity"
                              >
                                <div>
                                  <div className="flex items-center gap-1.5">
                                    <span className="font-semibold text-white group-hover/entity:text-cyan-300 transition-colors">
                                      {entity.title}
                                    </span>
                                    {entity.badge && (
                                      <span className="rounded bg-cyan-500/20 border border-cyan-500/30 px-1 py-0.2 text-[9px] font-bold text-cyan-300">
                                        {entity.badge}
                                      </span>
                                    )}
                                  </div>
                                  {entity.subtitle && (
                                    <p className="text-[10px] text-slate-400 mt-0.5">{entity.subtitle}</p>
                                  )}
                                </div>
                                <ArrowUpRight className="h-3.5 w-3.5 text-slate-500 group-hover/entity:text-cyan-400 shrink-0" />
                              </a>
                            ))}
                          </div>
                        </div>
                      ...
                      {msg.metadata?.action && (
                        <div className="mt-3 pt-2.5 border-t border-white/10">
                          <div className="p-3 rounded-xl bg-slate-950 border border-cyan-500/40 shadow-inner space-y-2">
                            <div className="flex items-center justify-between">
                              <span className="font-bold text-cyan-300 flex items-center gap-1.5 text-xs">
                                <Sparkles className="h-3.5 w-3.5" /> Action Confirmation
                              </span>
                              <span
                                className={cn(
                            ...
                            <p className="text-[11px] text-slate-300">{msg.metadata.action.description}</p>
                            ...
                            {msg.metadata.action.status === 'EXECUTED' ? (
                              ...
                              <div className="flex items-center gap-2 pt-1">
                                <button
                                  ...
                                  className="flex items-center gap-1 rounded-lg bg-cyan-500 px-3 py-1.5 text-xs font-bold text-black hover:bg-cyan-400 transition-colors disabled:opacity-50"
                                >
                      ...
                      {msg.metadata?.citations && msg.metadata.citations.length > 0 && (
                        <div className="mt-2.5 pt-1.5 border-t border-white/5 flex flex-wrap gap-2 text-[10px] text-slate-400">
                          {msg.metadata.citations.map((cite, cIdx) => (
                            <span key={cIdx} className="inline-flex items-center gap-1 text-cyan-400/80">
                      ...
                      {isAssistant && (
                        <div className="mt-2 flex items-center justify-between pt-1 text-[10px] text-slate-500">
                          <button
                            type="button"
              ...
              {isTyping && (
                <div className="flex items-center gap-3">
                  <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-tr from-blue-600 to-cyan-500 text-white">
                    <Bot className="h-3.5 w-3.5" />
                  </div>
                  <div className="flex items-center gap-2 rounded-2xl bg-slate-900 border border-white/10 px-4 py-3 text-xs text-slate-300">
                    ...
                    <span className="ml-1 text-[11px] text-slate-400">Civic AI is thinking & querying database...</span>
                  </div>
                </div>
              )}
            ...
            <div className="border-t border-white/5 px-4 py-2.5 bg-slate-950/60">
              ...
              <div className="flex flex-wrap gap-1.5">
                {getContextPrompts().map((prompt) => (
                  <button
                    ...
                    className="rounded-lg border border-white/10 bg-slate-900/90 px-2.5 py-1 text-[11px] text-slate-300 transition-colors hover:border-cyan-500/40 hover:bg-slate-800 hover:text-cyan-300 text-left"
                  >
                    {prompt}
                  </button>
                ))}
            ...
            <div className="border-t border-white/10 p-3 sm:p-4 bg-slate-900/95">
              ...
              <div className="mt-1.5 flex items-center justify-between text-[10px] text-slate-500 px-1">
                ...
                <span className="flex items-center gap-1 text-cyan-400/80">
                  <ShieldCheck className="h-3 w-3" /> Grounded in CivicSolve Registry
                </span>
              </div>
            </div>
          </motion.aside>
